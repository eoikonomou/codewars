...
Input
The input data consist of an array which contains at least 1 name, and single integer n which may go as high as the biggest number your
...
//   return whoIsNext(names.reduce((acc, val) => acc.concat(val, val), []), r - names.length);
// }

function whoIsNext(names, r) {
  let factor = 1;
  while (r > factor * names.length) {
    r -= factor * names.length;
    factor *= 2;
  }
  return names[Math.ceil(r / factor) - 1];
}
